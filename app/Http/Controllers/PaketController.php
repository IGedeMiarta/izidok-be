...
class PaketController extends Controller
{
    ...
    public function generatePdfInvoice(Request $request)
    {
        ...
        $pdf->setWatermarkImage(public_path('upload/images/lunas.png'));
        $pdf->loadHTML($html);
    ...
    public function checkPackage(){
        ...
        if ($paket->exists()) {
            ...
            }elseif (($pkt->limit <= 0 && $pkt->expired_date < date('Y-m-d H:i:s') && $billing->exists()) || ($pkt->limit == 0 && $pkt->expired_date > date('Y-m-d H:i:s') && $billing->exists()) || ($pkt->expired_date < date('Y-m-d H:i:s') && $billing->exists())) {
